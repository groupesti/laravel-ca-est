...
`laravel-ca-est` implements an RFC 7030-compliant EST server for certificate enrollment, re-enrollment, and server-side key generation over HTTPS. EST provides a modern, REST-based alternative to SCEP for certificate management. It supports HTTP Basic and TLS client certificate authentication. It depends on `laravel-ca` (core), `laravel-ca-crt` (certificate issuance), `laravel-ca-csr` (CSR processing), `laravel-ca-key` (key generation), and `laravel-ca-log` (structured logging).
...
**EstServer** -- Implements the four core EST operations: `/cacerts` (distribute CA certificates), `/simpleenroll` (initial enrollment from PKCS#10 CSR), `/simplereenroll` (certificate renewal), and `/serverkeygen` (server-side key generation with PKCS#7 response). Coordinates with the certificate, CSR, and key managers for actual PKI operations. All operations are logged via `CaLog::estOperation()`, and errors are logged via `CaLog::critical()`.

**EstAuthenticator** -- Handles EST client authentication. Supports HTTP Basic authentication (username/password) and TLS client certificate authentication (extracting the client cert from the TLS handshake). The authentication method is configurable. Authentication attempts are logged via `CaLog::estOperation()`.

**EstResponseBuilder** -- Formats EST responses according to RFC 7030. Wraps certificates in PKCS#7 certs-only structures, handles Base64 encoding of PKCS#10 CSRs, and sets appropriate Content-Type headers. Response building operations are logged via `CaLog::estOperation()`.
