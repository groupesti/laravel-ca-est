...
- Integrated `CA\Log` (`groupesti/laravel-ca-log`) for structured logging of all EST operations.
- All EST server operations (`ca_certs`, `simple_enroll`, `simple_reenroll`, `server_keygen`, `full_cmc`, `csr_attrs`) are now logged via `CaLog::estOperation()`.
- EST authentication attempts (HTTP Basic and client certificate) are now logged via `CaLog::estOperation()`.
- EST response building operations are now logged via `CaLog::estOperation()`.
- Critical errors in EST operations are now logged via `CaLog::critical()` with exception class context, in addition to the existing Laravel `Log` calls.
